...
describe('route extractor', () => {
    it('should extract routes from config', function () {
        ...
        const config: any = {
            ...
            pages: [
                {route: '/root/page-1', name: 'page-1', pageComponent: () => null, skip: false},
                {route: '/root/page-2', name: 'page-2', pageComponent: () => null, skip: false}
            ],
        };
        ...
        expect(routes[3].key).toEqual('page-1');
        expect(routes[3].props.path).toEqual('/root/page-1');
    ...
    it('should skip route with skip property set to true', function () {
        const config: any = {
            ...
            pages: [
                {route: '/root/page-1', name: 'page-1', pageComponent: () => null, skip: false},
                {route: '/root/page-2', name: 'page-2', pageComponent: () => null, skip: true}
            ],
        };
        const context: any = {config: config};
        const extractor = new RoutesExtractor(context);
